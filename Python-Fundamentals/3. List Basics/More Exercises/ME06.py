...
def exchange(arr, index):
    if index < len(arr) and index >= 0:
        return arr[index+1:] + arr[:index + 1]  # returns switched list
    else:
...
while True:
    ...
    if command == "end":
        ...
        if tokens[0] == "exchange":
            temp = exchange(data_list, int(tokens[1]))
            error = "Invalid index"
            if temp != error:
                data_list = temp
            else:
            ...
            if tokens[1] == "even":
                print(max_even(data_list))
            else:
                print(max_odd(data_list))
        elif tokens[0] == "min":
            if tokens[1] == "even":
                print(min_even(data_list))
            else:
                print(min_odd(data_list))
        elif tokens[0] == "first":
            count = int(tokens[1])
            if tokens[2] == "even":
                print(count_first_even(data_list, count))
            else:
                print(count_first_odd(data_list, count))
        elif tokens[0] == "last":
            count = int(tokens[1])
            if tokens[2] == "even":
                print(count_last_even(data_list, count))
            else:
                print(count_last_odd(data_list, count))
